...
build-wasm: ## Compile Zig to WebAssembly
	...
	cp zig/zig-out/bin/diff.wasm wasm/diff.wasm

types: ## Generate TypeScript declarations from JSDoc
	npx tsc --declaration --allowJs --emitDeclarationOnly --skipLibCheck --target es2020 --module nodenext --moduleResolution nodenext --strict false --esModuleInterop true --outDir ./types src/index.js src/fallback.js src/wasm.js src/native.js

clean: ## Remove build artifacts
	rm -rf zig/zig-out zig/.zig-cache wasm/diff.wasm types/
